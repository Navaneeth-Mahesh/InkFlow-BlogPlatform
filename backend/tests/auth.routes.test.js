...
describe("POST /api/v1/auth/register", () => {
    it("rejects a request missing required fields", async () => {
        const res = await (0, supertest_1.default)(app).post("/api/v1/auth/register").send({ email: "a@b.com" });
...
describe("POST /api/v1/auth/login", () => {
    it("rejects when user does not exist", async () => {
        ...
        const res = await (0, supertest_1.default)(app).post("/api/v1/auth/login").send({ email: "nouser@example.com", password: "password123" });
    ...
    it("logs in successfully with correct credentials", async () => {
        ...
        const res = await (0, supertest_1.default)(app).post("/api/v1/auth/login").send({ email: "sofia@inkflow.app", password: "password123" });
...
describe("404 handling", () => {
    it("returns a standardized 404 for unknown routes", async () => {
        const res = await (0, supertest_1.default)(app).get("/api/v1/this-route-does-not-exist");
